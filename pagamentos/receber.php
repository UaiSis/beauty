...
<div id="info_pagamento" style="text-align: center;"> 
        <p class="h3 font-weight-normal" style=" font-size: 18px; border-radius: 4px;"><span>(<?=$descricao;?>)</span> <span style="color:green; ">R$ <?=$valorF;?></span> </p>
</div>    
            ...
            const settings = {
                ...
                // receber o resultado do pagamento
                                if(response.status==true){
                                    window.location.href = "<?=$url_sistema;?>pagamentos/receber.php?id="+response.id+'&id_conta='+id_conta;
                                }
                                if(response.status!=true){
...
function check(id, id_conta) {
    var settings = {
        "url": "<?=$url_sistema;?>pagamentos/process_payment_conta.php?acc=check&id=" + id + "&id_conta=" + id_conta,
        "method": "GET",
        "timeout": 0
    };
