...
<div class="page-content header-clear-medium">
  ...
  <div class="card card-style">
    <div class="content">
      <?php
     $query = $pdo->query("SELECT * FROM dias where funcionario = '$id_func' ORDER BY id asc");
          ...
          <div class="splide__list">
            <div class="splide__slide mx-3">
              <div class="d-flex">
                ...
                <div>
                <h5 class="mt-1 mb-0" style="font-size: 13px;"> {$dia}</h5>
                <p class="font-12 mt-n2 mb-0 ">{$inicio} / {$final}</p>
                <p class="font-12 mt-n2 mb-0 color-blue-dark">{$inicio_almoco} / {$final_almoco}</p>
            ...
            <div class="splide__slide mx-3">
              ...
              <div class="d-flex">
                <div class="ms-auto">
                  ...
                  <a onclick="editarDias('{$id}','{$dia}', '{$inicio}', '{$final}', '{$inicio_almoco}', '{$final_almoco}')" href="#" class="icon icon-xs rounded-circle shadow-l bg-twitter"><i class="fa fa-edit text-white"></i></a>                  

                  <a onclick="excluir_dias('{$id}', '{$dia}')" href="#" class="icon icon-xs rounded-circle shadow-l bg-google"><i class="bi bi-trash-fill text-white"></i></a>           
...
<div class="offcanvas offcanvas-top rounded-m offcanvas-detached" style="height:98%" id="popupForm">
  <div class="content mb-4">
    ...
    <form id="form_dias" method="post" class="demo-animation needs-validation m-0">
  ...
  <div class="col-12">
    <div class="form-floating mb-3 position-relative">
      ...
      <select class="form-control rounded-xs ps-5 pe-5" id="dias" name="dias" required>
      ...
      <label class="color-theme ps-5">Dia da Semana</label>
    </div>
  </div>

  <!-- Jornada de Trabalho (Início e Final) -->
  <div class="col-6">
    <div class="form-floating mb-3 position-relative">
      <i class="bi bi-clock-fill position-absolute start-0 top-50 translate-middle-y ms-3"></i>
      <input type="time" class="form-control rounded-xs ps-5" id="inicio" name="inicio" required>
      <label class="color-theme ps-5">Início Jornada</label>
    </div>
  </div>

  <div class="col-6">
    <div class="form-floating mb-3 position-relative">
      <i class="bi bi-clock-history position-absolute start-0 top-50 translate-middle-y ms-3"></i>
      <input type="time" class="form-control rounded-xs ps-5" id="final" name="final" required>
      <label class="color-theme ps-5">Final Jornada</label>
    </div>
  </div>

  <!-- Intervalo de Almoço (Início e Final) -->
  <div class="col-6">
    <div class="form-floating mb-3 position-relative">
      <i class="bi bi-sunrise position-absolute start-0 top-50 translate-middle-y ms-3"></i>
      <input type="time" class="form-control rounded-xs ps-5" id="inicio_almoco" name="inicio_almoco">
      <label class="color-theme ps-5">Início Almoço</label>
    </div>
  </div>

  <div class="col-6">
    <div class="form-floating mb-3 position-relative">
      <i class="bi bi-sunset position-absolute start-0 top-50 translate-middle-y ms-3"></i>
      <input type="time" class="form-control rounded-xs ps-5" id="final_almoco" name="final_almoco">
      <label class="color-theme ps-5">Final Almoço</label>
    </div>
  </div>
...
function excluir_dias(id) {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: '../../sistema/painel/paginas/' + pag + "/excluir-dias.php",
                method: 'POST',
                data: { id },
                dataType: "html",
