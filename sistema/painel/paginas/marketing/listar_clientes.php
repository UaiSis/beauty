...
if ($clientes == "Teste") {
	$resultado = $pdo->query("SELECT telefone_whatsapp FROM config where telefone_whatsapp != ''");
	...
	$resultado = $pdo->query("SELECT telefone FROM clientes where telefone != '' and MONTH(data_nasc) = '$dataMes' and (marketing = '' or marketing is null)");
	...
	$resultado = $pdo->query("SELECT DISTINCT c.nome, c.telefone, c.id FROM clientes c JOIN receber r ON c.id = r.pessoa WHERE r.data_venc < CURDATE() AND r.pago = 'Não'");
