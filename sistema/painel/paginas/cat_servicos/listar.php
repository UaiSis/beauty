...
<tr>
<td>
	<div class="categoria-info-cell">
		<div class="categoria-icon">
			<i class="fa {$icone}"></i>
		</div>
		<div class="categoria-name">{$nome}</div>
	</div>
</td>
<td>
	<span class="count-badge">{$total_servicos}</span>
</td>
<td>
...
HTML;  
	...
	echo '<div class="empty-state">
		...
		<small>Adicione a primeira categoria clicando no botão "Nova Categoria"</small>
	</div>';
}
	...
	function renderMobileCards() {
		...
		if($total_reg_mobile > 0){
			for($i=0; $i < $total_reg_mobile; $i++){
				...
				// Contar serviços
				$query_serv = $pdo->query("SELECT * FROM servicos where categoria = '$id'");
				$res_serv = $query_serv->fetchAll(PDO::FETCH_ASSOC);
				$total_servicos = @count($res_serv);
				
				echo "mobileHtml += `";
				echo "<div class='categoria-card-mobile'>";
				echo "<div class='categoria-card-header'>";
				echo "<div class='categoria-card-icon'><i class='fa {$icone}'></i></div>";
				echo "<div class='categoria-card-info'>";
				echo "<div class='categoria-card-name'>{$nome}</div>";
				echo "<div class='categoria-card-count'>{$total_servicos} serviços</div>";
				echo "</div>";
				echo "</div>";
				
				echo "<div class='categoria-card-actions'>";
				echo "<a href='#' onclick=\"editar('{$id}','{$nome}', '{$icone}')\" class='categoria-card-action-btn edit'>";
