...
<tr>
<td>
	<div class="cargo-info-cell">
		<div class="cargo-icon">
			<i class="fa {$icone}"></i>
		</div>
		<div class="cargo-name">{$nome}</div>
	</div>
</td>
<td>
	<div class="table-actions-cell">
		...
		<a href="#" onclick="confirmarExclusaoCargo('{$id}')" class="table-action-icon delete" title="Excluir">
...
<div class="pagination-modern">
	<ul style="display: flex; gap: 8px; padding: 0; margin: 0; list-style: none;">
		<li class="page-item">
			<a onclick="listarCargos(0)" class="paginador" href="#" aria-label="Previous">
...
echo <<<HTML
		<li class="page-item {$estilo}">
			<a onclick="listarCargos({$i})" class="paginador" href="#">{$pag_num}</a>
		</li>
HTML;
		} 
	} 
...
echo <<<HTML
		<li class="page-item">
			<a onclick="listarCargos({$ultimo_reg})" class="paginador" href="#" aria-label="Next">
...
HTML;  
	...
	echo '<div class="empty-state">
		...
		<small>Adicione o primeiro cargo clicando no botão "Novo Cargo"</small>
	</div>';
}
	...
	function renderMobileCards() {
		...
		if($total_reg_mobile > 0){
			for($i=0; $i < $total_reg_mobile; $i++){
				...
				echo "mobileHtml += `";
				echo "<div class='cargo-card-mobile'>";
				echo "<div class='cargo-card-header'>";
				echo "<div class='cargo-card-icon'><i class='fa {$icone}'></i></div>";
				echo "<div class='cargo-card-info'>";
				echo "<div class='cargo-card-name'>{$nome}</div>";
				echo "</div>";
				echo "</div>";
				
				echo "<div class='cargo-card-actions'>";
				echo "<a href='#' onclick=\"editar('{$id}','{$nome}', '{$icone}')\" class='cargo-card-action-btn edit'>";
				echo "<i class='fa fa-edit'></i> Editar";
				echo "</a>";
				echo "<a href='#' onclick=\"confirmarExclusaoCargo('{$id}')\" class='cargo-card-action-btn delete'>";
...
<script type="text/javascript">
	function confirmarExclusaoCargo(id){
		...
		setTimeout(function(){
			$('#mensagem-excluir').html(`
				<span style="margin-right: 16px;">Tem certeza que deseja excluir este cargo?</span>
