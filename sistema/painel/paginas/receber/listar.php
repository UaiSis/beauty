...
<td>
	<div class="table-actions-cell">
		<a href="#" onclick="editar('{$id}','{$descricao}', '{$pessoa}', '{$valor}', '{$data_venc}', '{$data_pgto}', '{$tumb_arquivo}', '{$pgto}', '{$servico}', '{$frequencia}')" class="table-action-icon edit" title="Editar">
			<i class="fa fa-edit"></i>
		</a>

		<a href="#" onclick="mostrar('{$descricao}', '{$valorF}', '{$data_lancF}', '{$data_vencF}',  '{$data_pgtoF}', '{$nome_usuario_lanc}', '{$nome_usuario_pgto}', '{$tumb_arquivo}', '{$nome_pessoa}', '{$foto}', '{$pgto}', '{$nome_servico}', '{$frequencia}')" class="table-action-icon view" title="Visualizar">
			<i class="fa fa-eye"></i>
		</a>
...
<script type="text/javascript">
	...
	function renderMobileCards() {
		...
		var mobileHtml = '';
		<?php 
		$query_mobile = $pdo->query("SELECT * FROM receber where data_venc >= '$dataInicial' and data_venc <= '$dataFinal' and pago LIKE '$status' ORDER BY pago asc, data_venc asc");
		...
		if($total_reg > 0){
			for($i=0; $i < $total_reg; $i++){
				...
				echo "</div>";
				echo "<div class='conta-card-detail-item'>";
				echo "<div class='conta-card-detail-label'>Cliente</div>";
				echo "<div class='conta-card-detail-value'>{$nome_pessoa}</div>";
				echo "</div>";
				echo "<div class='conta-card-detail-item'>";
				echo "<div class='conta-card-detail-label'>Serviço</div>";
				echo "<div class='conta-card-detail-value'>{$nome_servico}</div>";
				echo "</div>";
				echo "</div>";
				
				echo "<div class='conta-card-actions'>";
				echo "<a href='#' onclick=\"editar('{$id}','{$descricao}', '{$pessoa}', '{$valor}', '{$data_venc}', '{$data_pgto}', '{$tumb_arquivo}', '{$pgto}', '{$servico}', '{$frequencia}')\" class='conta-card-action-btn edit'>";
				echo "<i class='fa fa-edit'></i> Editar";
				echo "</a>";
				
				echo "<a href='#' onclick=\"mostrar('{$descricao}', '{$valorF}', '{$data_lancF}', '{$data_vencF}', '{$data_pgtoF}', '{$nome_usuario_lanc}', '{$nome_usuario_pgto}', '{$tumb_arquivo}', '{$nome_pessoa}', '{$foto}', '{$pgto}', '{$nome_servico}', '{$frequencia}')\" class='conta-card-action-btn view'>";
				...
				if($tem_whatsapp){
					echo "<a href='http://api.whatsapp.com/send?1=pt_BR&phone={$whats_mobile}&text=' target='_blank' class='conta-card-action-btn whatsapp' style='flex: 0;'>";
...
<script type="text/javascript">
	function editar(id, descricao, pessoa, valor, data_venc, data_pgto, foto, pgto, servico, frequencia){
		$('#id').val(id);
...
<script type="text/javascript">
	function mostrar(descricao, valor, data_lanc, data_venc, data_pgto, usuario_lanc, usuario_pgto, foto, pessoa, link, pgto, servico, frequencia){
